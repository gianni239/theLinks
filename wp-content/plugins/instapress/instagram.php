	...
	*/
	...
	class InstagramPlugin
	{
		...
		function shortcode($params)
		{
			...
			$imageHtml = '<img class="journal-image" src="'.get_bloginfo( 'wpurl' ).'/wp-content/themes/sqd/_/img/journal-overlay.png" ><img src="%1$s" ';
				...
				if(count($data) > 0)
				{
					foreach($data as $obj)
					{
						...
						switch($values['effect'])
						{
							// jQuery Fancybox
							case 'fancybox':
								$result .= '<a class="fancybox" href="'.$obj->images->standard_resolution->url.'" rel="gallery" title="'.$title.'">';
								break;
							case 'highslide':
